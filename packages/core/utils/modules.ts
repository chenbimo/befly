 ...
 * @returns 模块列表
 */
export async function scanModules<T extends Plugin | Hook>(
  dir: string,
  type: "core" | "addon" | "app",
  moduleLabel: string,
  addonName?: string,
  ...
  for (const { filePath, fileName } of files) {
    ...
    const moduleName =
      type === "core"
        ? name
        : type === "addon"
          ? `addon_${camelCase(addonName!)}_${name}`
          : `app_${name}`;
 ...
 */
export function sortModules<T extends { name?: string; after?: string[] }>(
  modules: T[],
): T[] | false {
  const result: T[] = [];
