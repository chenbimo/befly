...
core 仅支持 **MySQL 8.0 及以上**。

数据库连接由配置 `db` 决定（见 `config.md`），运行时通过 `ctx.db` 提供数据库能力。
...
## 表结构同步（syncTable）与类型映射（MySQL）

### 从 table.json 到真实表结构

`syncTable` 会把 `tables/*.json` 中的 FieldDefinition 同步为数据库列（MySQL 8.0+）：

- 列名：`snakeCase(fieldKey)`
- 注释：会写入 MySQL 的 `COMMENT`
