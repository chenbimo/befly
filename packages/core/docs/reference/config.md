...
// befly.production.json 配置按以下顺序加载，后加载的覆盖先加载的：
...
> 注意：`db.dialect` 支持填写 `postgres`，框架会在内部归一化为 `postgresql`。

| 配置项     | 类型   | 默认值         | 说明                                  |
| ---------- | ------ | -------------- | ------------------------------------- |
| `dialect`  | string | `'mysql'`      | 数据库方言（mysql/postgresql/sqlite） |
...
-   字符串：`'7d'`（7 天）、`'24h'`（24 小时）、`'60m'`（60 分钟）
-   数字：秒数，如 `604800`（7 天）
...
export default {
    ...
    handler: async (befly, ctx) => {
        ...
        const dbDialect = befly.config.db?.dialect;
