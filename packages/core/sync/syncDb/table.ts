...
export async function modifyTable(
    ...
    if (existingColumns[dbFieldName]) {
      ...
      if (comparison.length > 0) {
        for (const c of comparison) {
          // 使用统一的日志格式函数和常量标签
          const changeLabel =
            CHANGE_TYPE_LABELS[c.type as keyof typeof CHANGE_TYPE_LABELS] || "未知";
          logFieldChange(tableName, dbFieldName, c.type, c.current, c.expected, changeLabel);
        }

        if (
          isStringOrArrayType(fieldDef.type) &&
          existingColumns[dbFieldName].max &&
          fieldDef.max !== null
        ) {
          if (existingColumns[dbFieldName].max! > fieldDef.max) {
            if (force) {
              Logger.warn(
                `[强制执行] ${tableName}.${dbFieldName} 长度收缩 ${existingColumns[dbFieldName].max} -> ${fieldDef.max}`,
              );
            } else {
              Logger.warn(
                `[跳过危险变更] ${tableName}.${dbFieldName} 长度收缩 ${existingColumns[dbFieldName].max} -> ${fieldDef.max} 已被跳过（使用 --force 强制执行）`,
              );
            }
          }
        }
        ...
        if (hasTypeChange) {
          ...
          if (!isCompatibleTypeChange(currentType, expectedType)) {
            const errorMsg = [
              ...
              `说明: 仅允许宽化型变更（如 INT->BIGINT, VARCHAR->TEXT），其他类型变更需要手动处理`,
            ].join("\n");
            throw new Error(errorMsg);
          }
          Logger.debug(
            `[兼容类型变更] ${tableName}.${dbFieldName} ${currentType} -> ${expectedType}`,
          );
        }
  ...
  for (const [fieldKey, fieldDef] of Object.entries(fields)) {
    ...
    if (fieldDef.index && !fieldDef.unique && !existingIndexes[indexName]) {
      ...
      existingIndexes[indexName].length === 1
    ) {
    ...
    for (const [fieldKey, fieldDef] of Object.entries(fields)) {
      ...
      if (existingColumns[dbFieldName]) {
        ...
        if (want !== curr) {
          ...
          commentActions.push(
            `COMMENT ON COLUMN "${tableName}"."${dbFieldName}" IS ${want ? `'${escapedWant}'` : "NULL"}`,
          );
          changed = true;
        }
      }
  ...
  changed =
    addClauses.length > 0 ||
    modifyClauses.length > 0 ||
    defaultClauses.length > 0 ||
    indexActions.length > 0 ||
