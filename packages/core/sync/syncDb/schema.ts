...
export async function tableExists(sql: SQL, tableName: string, dbName: string): Promise<boolean> {
  ...
  try {
    if (isMySQL()) {
      const res =
        await sql`SELECT COUNT(*) AS count FROM information_schema.TABLES WHERE TABLE_SCHEMA = ${dbName} AND TABLE_NAME = ${tableName}`;
      return (res[0]?.count || 0) > 0;
    }

    if (isPG()) {
      const res =
        await sql`SELECT COUNT(*)::int AS count FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ${tableName}`;
      return (res[0]?.count || 0) > 0;
    }

    if (isSQLite()) {
      const res =
        await sql`SELECT name FROM sqlite_master WHERE type='table' AND name = ${tableName}`;
      return res.length > 0;
    }
