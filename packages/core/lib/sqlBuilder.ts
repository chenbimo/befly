...
export class SqlBuilder {
  ...
  private _applyOperator(fieldName: string, operator: WhereOperator, value: any): void {
    ...
    switch (operator) {
      ...
      case "$in":
        ...
        if (value.length === 0) {
          throw new Error(
            `$in 操作符的数组不能为空。提示：空数组会导致查询永远不匹配任何记录，这通常不是预期行为。请检查查询条件或移除该字段。`,
          );
        }
      ...
      case "$notIn":
        ...
        if (value.length === 0) {
          throw new Error(
            `$nin/$notIn 操作符的数组不能为空。提示：空数组会导致查询匹配所有记录，这通常不是预期行为。请检查查询条件或移除该字段。`,
          );
        }
  ...
  orderBy(fields: string[]): this {
    ...
    fields.forEach((item) => {
      if (typeof item !== "string" || !item.includes("#")) {
        throw new Error(
          `orderBy 字段必须是 "字段#方向" 格式的字符串（例如："name#ASC", "id#DESC"） (item: ${item})`,
        );
      }
  ...
  groupBy(field: string | string[]): this {
    if (Array.isArray(field)) {
      const escapedFields = field
        .filter((f) => typeof f === "string")
        .map((f) => this._escapeField(f));
