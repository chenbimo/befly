    ...
    for (const item of allTableFiles) {
      ...
      try {
        ...
        if (!LOWER_CAMEL_CASE_REGEX.test(fileBaseName)) {
          Logger.warn(
            `${item.typeName}表 ${fileName} 文件名必须使用小驼峰命名（例如 testCustomers.json）`,
          );
          hasError = true;
          continue;
        }
        ...
        for (const [colKey, fieldDef] of Object.entries(table)) {
          ...
          if (RESERVED_FIELDS.includes(colKey as any)) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件包含保留字段 ${colKey}，` +
                `不能在表定义中使用以下字段: ${RESERVED_FIELDS.join(", ")}`,
            );
            hasError = true;
          }
          ...
          const illegalProps = fieldKeys.filter(
            (key) => !ALLOWED_FIELD_PROPERTIES.includes(key as any),
          );
          if (illegalProps.length > 0) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 包含非法属性: ${illegalProps.join(", ")}，` +
                `允许的属性为: ${ALLOWED_FIELD_PROPERTIES.join(", ")}`,
            );
            hasError = true;
          }
          ...
          if (!field.name || typeof field.name !== "string") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 缺少必填字段 name 或类型错误`,
            );
            hasError = true;
            continue;
          }
          if (!field.type || typeof field.type !== "string") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 缺少必填字段 type 或类型错误`,
            );
            hasError = true;
            continue;
          }
          ...
          if (field.min !== undefined && !(field.min === null || typeof field.min === "number")) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 min 类型错误，必须为 null 或数字`,
            );
            hasError = true;
          }
          if (field.max !== undefined && !(field.max === null || typeof field.max === "number")) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 max 类型错误，必须为 null 或数字`,
            );
            hasError = true;
          }
          if (field.detail !== undefined && typeof field.detail !== "string") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 detail 类型错误，必须为字符串`,
            );
            hasError = true;
          }
          if (field.index !== undefined && typeof field.index !== "boolean") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 index 类型错误，必须为布尔值`,
            );
            hasError = true;
          }
          if (field.unique !== undefined && typeof field.unique !== "boolean") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 unique 类型错误，必须为布尔值`,
            );
            hasError = true;
          }
          if (field.nullable !== undefined && typeof field.nullable !== "boolean") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 nullable 类型错误，必须为布尔值`,
            );
            hasError = true;
          }
          if (field.unsigned !== undefined && typeof field.unsigned !== "boolean") {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 unsigned 类型错误，必须为布尔值`,
            );
            hasError = true;
          }
          if (
            ...
            field.regexp !== null &&
            typeof field.regexp !== "string"
          ) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段 regexp 类型错误，必须为 null 或字符串`,
            );
            hasError = true;
          }
          ...
          if (!FIELD_NAME_REGEX.test(fieldName)) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段名称 "${fieldName}" 格式错误，` +
                `必须为中文、数字、字母、下划线、短横线、空格`,
            );
            hasError = true;
          }
          ...
          if (!FIELD_TYPES.includes(fieldType as any)) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 字段类型 "${fieldType}" 格式错误，` +
                `必须为${FIELD_TYPES.join("、")}之一`,
            );
            hasError = true;
          }
          ...
          if (field.unique && field.index) {
            Logger.warn(
              `${item.typeName}表 ${fileName} 文件 ${colKey} 同时设置了 unique=true 和 index=true，` +
                `unique 约束会自动创建唯一索引，index=true 将被忽略以避免重复索引`,
            );
          }
          ...
          if (
            ...
            if (fieldMin > fieldMax) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 最小值 "${fieldMin}" 不能大于最大值 "${fieldMax}"`,
              );
              hasError = true;
            }
          }
          ...
          if (fieldType === "text") {
            ...
            if (fieldMin !== undefined && fieldMin !== null) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 的 text 类型最小值应为 null，当前为 "${fieldMin}"`,
              );
              hasError = true;
            }
            if (fieldMax !== undefined && fieldMax !== null) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 的 text 类型最大长度应为 null，当前为 "${fieldMax}"`,
              );
              hasError = true;
            }
            if (fieldDefault !== undefined && fieldDefault !== null) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 为 text 类型，默认值必须为 null，当前为 "${fieldDefault}"`,
              );
              hasError = true;
            }
          } else if (
            ...
            if (fieldMax !== undefined && (fieldMax === null || typeof fieldMax !== "number")) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 为 ${fieldType} 类型，` +
                  `最大长度必须为数字，当前为 "${fieldMax}"`,
              );
              hasError = true;
            } else if (fieldMax !== undefined && fieldMax > MAX_VARCHAR_LENGTH) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 最大长度 ${fieldMax} 越界，` +
                  `${fieldType} 类型长度必须在 1..${MAX_VARCHAR_LENGTH} 范围内`,
              );
              hasError = true;
            }
            ...
            if (
              ...
              fieldDefault !== null &&
              typeof fieldDefault !== "number"
            ) {
              Logger.warn(
                `${item.typeName}表 ${fileName} 文件 ${colKey} 为 number 类型，` +
                  `默认值必须为数字或 null，当前为 "${fieldDefault}"`,
              );
              hasError = true;
            }
          }
