...
describe("buildIndexSQL (MySQL)", () => {
    test("创建索引 SQL", () => {
        const sql = SyncTable.buildIndexSQL("mysql", "user", "idx_created_at", "created_at", "create");
    ...
    test("删除索引 SQL", () => {
        const sql = SyncTable.buildIndexSQL("mysql", "user", "idx_created_at", "created_at", "drop");
        expect(sql).toContain("ALTER TABLE `user`");
...
describe("buildSystemColumnDefs (MySQL)", () => {
    test("返回 5 个系统字段定义", () => {
        const defs = SyncTable.buildSystemColumnDefs("mysql");
...
describe("buildBusinessColumnDefs (MySQL)", () => {
    test("生成 string 类型字段", () => {
        ...
        const defs = SyncTable.buildBusinessColumnDefs("mysql", fields as any);
...
describe("generateDDLClause (MySQL)", () => {
    test("生成 ADD COLUMN 子句", () => {
        ...
        const clause = SyncTable.generateDDLClause("mysql", "userName", fieldDef as any, true);
    ...
    test("生成 MODIFY COLUMN 子句", () => {
        ...
        const clause = SyncTable.generateDDLClause("mysql", "userName", fieldDef as any, false);
...
describe("isCompatibleTypeChange", () => {
    test("varchar -> text 是兼容变更", () => {
        expect(SyncTable.isCompatibleTypeChange("character varying", "text")).toBe(true);
    ...
    test("text -> varchar 不是兼容变更", () => {
        expect(SyncTable.isCompatibleTypeChange("text", "character varying")).toBe(false);
    ...
    test("PG integer -> bigint 是兼容变更", () => {
        expect(SyncTable.isCompatibleTypeChange("integer", "bigint")).toBe(true);
        expect(SyncTable.isCompatibleTypeChange("smallint", "integer")).toBe(true);
        expect(SyncTable.isCompatibleTypeChange("smallint", "bigint")).toBe(true);
    });
    ...
    test("空值处理", () => {
        expect(SyncTable.isCompatibleTypeChange(null as any, "text")).toBe(false);
        expect(SyncTable.isCompatibleTypeChange("text", null as any)).toBe(false);
    });
});
