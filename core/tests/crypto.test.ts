...
describe('SHA256 哈希', () => {
    ...
    test('sha256 应该支持十六进制编码', () => {
        const hash = Crypto.sha256('test', 'hex');
        expect(hash).toMatch(/^[a-f0-9]{64}$/);
    });

    test('sha256 应该支持 Base64 编码', () => {
        const hash = Crypto.sha256('test', 'base64');
...
describe('HMAC', () => {
    test('hmacMd5 应该返回正确的 HMAC', () => {
        const hmac = Crypto.hmacMd5('message', 'secret');
    ...
    test('相同输入应该产生相同 HMAC', () => {
        const hmac1 = Crypto.hmacMd5('test', 'key');
        const hmac2 = Crypto.hmacMd5('test', 'key');
        expect(hmac1).toBe(hmac2);
    });

    test('不同密钥应该产生不同 HMAC', () => {
        const hmac1 = Crypto.hmacMd5('test', 'key1');
        const hmac2 = Crypto.hmacMd5('test', 'key2');
...
describe('密码加密', () => {
    test('hashPassword 应该返回加密后的密码', async () => {
        const hashed = await Crypto.hashPassword('password123');
    ...
    test('verifyPassword 应该拒绝错误的密码', async () => {
        const password = 'mypassword';
        const hashed = await Crypto.hashPassword(password);
        const isValid = await Crypto.verifyPassword('wrongpassword', hashed);
...
describe('Base64 编码', () => {
    ...
    test('base64Decode 应该正确解码', () => {
        const decoded = Crypto.base64Decode('aGVsbG8=');
...
describe('随机字符串', () => {
    ...
    test('多次调用应该返回不同的字符串', () => {
        const str1 = Crypto.randomString(16);
        const str2 = Crypto.randomString(16);
