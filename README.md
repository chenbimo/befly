...
请参考：`packages/core/docs/sync.md`（`syncDb` / `syncAll` 等同步流程说明）。
...
## 🔁 迁移提示：`befly-shared` 已移除

本仓库已完成对历史包 `befly-shared` 的迁移并将其从 workspace 中移除：

- **服务端/框架侧工具**：请从 `befly/lib/*` 导入
- **前端/Vite 环境工具**：请从 `befly-vite/utils/*` 导入

如果你在旧代码/旧文章中看到 `befly-shared/*`，请按以上归属替换为新的包路径。
